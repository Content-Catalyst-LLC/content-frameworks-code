"""Export utilities for institutional communication Catalyst Canvas integration."""
...
def build_canvas_cards(results: list[AuditResult]) -> list[dict[str, object]]:
    ...
    for result in results:
        cards.append({
            "card_id": f"institutional-communication-item-{slugify(result.item)}",
            "type": "institutional_communication_item_card",
            ...
            "badges": [
                ...
                f"trust risk: {result.trust_risk}",
                result.status,
            ],
            "metrics": {
                "quality_score": result.quality_score,
                "evidence_gap": result.evidence_gap,
                "trust_risk": result.trust_risk,
            ...
            "communication": {
                ...
                "cultural_alignment": result.cultural_alignment,
...
def build_catalog(results: list[AuditResult]) -> dict[str, object]:
    ...
    return {
        ...
        "outputs": {
            "audit_table": "outputs/tables/institutional_communication_canvas_audit.csv",
            "canvas_cards": "canvas/canvas_cards.json",
            "governance_queue": "canvas/governance_queue.json",
            "full_export": "outputs/json/institutional_communication_canvas_export.json"
        }
    }
...
def write_markdown_queue(path: Path, queue: list[dict[str, object]]) -> None:
    ...
    lines = [
        "# Institutional and Organizational Communication Catalyst Canvas Governance Queue",
        "",
        "Generated review queue for communication quality, evidence gaps, trust risk, authority coverage, feedback quality, and governance status.",
        "",
    ]
...
def export_all(article_root: Path, results: list[AuditResult]) -> None:
    ...
    write_csv(article_root / "outputs" / "tables" / "institutional_communication_canvas_audit.csv", result_dicts)
    ...
    write_json(article_root / "outputs" / "json" / "institutional_communication_canvas_export.json", full_export)
