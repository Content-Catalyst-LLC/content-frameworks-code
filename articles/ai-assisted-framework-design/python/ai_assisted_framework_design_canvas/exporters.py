"""Export utilities for AI-Assisted Framework Design Catalyst Canvas integration."""
...
def build_canvas_cards(results: list[AuditResult]) -> list[dict[str, object]]:
    ...
    for result in results:
        cards.append({
            "card_id": f"ai-assisted-framework-design-item-{slugify(result.item)}",
            "type": "ai_assisted_framework_design_item_card",
            "article_slug": ARTICLE_SLUG,
            "title": result.item,
            "subtitle": result.item_type,
            "description": result.description,
            "badges": [
                f"priority: {result.governance_priority}",
                f"readiness: {result.readiness_score}",
                f"AI risk: {result.ai_framework_risk}",
                result.status,
            ],
            "metrics": {
...
def build_catalog(results: list[AuditResult]) -> dict[str, object]:
    ...
    return {
        ...
        "outputs": {
            "audit_table": "outputs/tables/ai_assisted_framework_design_canvas_audit.csv",
            "canvas_cards": "canvas/canvas_cards.json",
            "governance_queue": "canvas/governance_queue.json",
            "full_export": "outputs/json/ai_assisted_framework_design_canvas_export.json"
        }
    }
...
def write_markdown_queue(path: Path, queue: list[dict[str, object]]) -> None:
    ...
    lines = [
        "# AI-Assisted Framework Design Catalyst Canvas Governance Queue",
        "",
        "Generated governance queue for conceptual clarity, evidence grounding, metadata consistency, human review strength, bias review, governance maturity, platform readiness, drift control, unsupported-claim risk, generic-structure risk, output validation, audience impact, and AI framework risk.",
        "",
    ]
    ...
    if not queue:
        ...
        for item in queue:
            lines.extend([
                f"## {item['item']}",
                "",
                f"- Priority: {item['governance_priority']}",
                f"- Item type: {item['item_type']}",
                f"- Owner: {item['owner']}",
                f"- Status: {item['status']}",
                f"- Review date: {item['review_date']}",
                f"- Readiness score: {item['readiness_score']}",
                f"- AI framework risk: {item['ai_framework_risk']}",
                f"- Governance priority score: {item['governance_priority_score']}",
...
def export_all(article_root: Path, results: list[AuditResult]) -> None:
    ...
    write_csv(article_root / "outputs" / "tables" / "ai_assisted_framework_design_canvas_audit.csv", result_dicts)
    ...
    write_json(article_root / "outputs" / "json" / "ai_assisted_framework_design_canvas_export.json", full_export)
