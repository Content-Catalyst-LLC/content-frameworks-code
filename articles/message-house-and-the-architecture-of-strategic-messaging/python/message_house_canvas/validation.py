"""Validation helpers for Message House Catalyst Canvas inputs."""
...
def validate_rows(rows: list[dict[str, str]]) -> list[str]:
    ...
    if not rows:
        return ["Input contains no message pillar rows."]
    ...
    for index, row in enumerate(rows, start=1):
        ...
        if pillar:
            normalized = pillar.casefold()
            if normalized in seen_pillars:
                errors.append(f"{row_label}: duplicate pillar '{pillar}'.")
            seen_pillars.add(normalized)
