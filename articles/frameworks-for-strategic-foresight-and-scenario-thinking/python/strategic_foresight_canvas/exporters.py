"""Export utilities for strategic foresight Catalyst Canvas integration."""
...
def build_canvas_cards(results: list[AuditResult]) -> list[dict[str, object]]:
    ...
    for result in results:
        cards.append({
            "card_id": f"strategic-foresight-item-{slugify(result.item)}",
            "type": "strategic_foresight_item_card",
            "article_slug": ARTICLE_SLUG,
            "title": result.item,
            "subtitle": result.foresight_type,
            ...
            "metrics": {
                ...
                "driver_clarity": result.driver_clarity,
                "uncertainty_logic": result.uncertainty_logic,
                "scenario_logic": result.scenario_logic,
                "assumption_transparency": result.assumption_transparency,
                "indicator_coverage": result.indicator_coverage
            },
            "foresight": {
                "foresight_type": result.foresight_type,
                "option_relevance": result.option_relevance,
...
def build_catalog(results: list[AuditResult]) -> dict[str, object]:
    foresight_type_counts: dict[str, int] = {}

    for result in results:
        foresight_type_counts[result.foresight_type] = foresight_type_counts.get(result.foresight_type, 0) + 1
    ...
    return {
        ...
        "foresight_type_counts": foresight_type_counts,
        "metrics": [
            "quality_score",
            "assumption_risk",
        ...
        "outputs": {
            "audit_table": "outputs/tables/strategic_foresight_canvas_audit.csv",
            "canvas_cards": "canvas/canvas_cards.json",
            "governance_queue": "canvas/governance_queue.json",
            "full_export": "outputs/json/strategic_foresight_canvas_export.json"
        }
    }
...
def write_markdown_queue(path: Path, queue: list[dict[str, object]]) -> None:
    ...
    lines = [
        "# Strategic Foresight and Scenario Thinking Catalyst Canvas Governance Queue",
        "",
        "Generated review queue for scenario quality, assumption risk, indicator coverage, option relevance, evidence strength, stakeholder visibility, and governance status.",
        "",
    ]
    ...
    if not queue:
        ...
        for item in queue:
            lines.extend([
                ...
                f"- Foresight type: {item['foresight_type']}",
                f"- Owner: {item['owner']}",
...
def export_all(article_root: Path, results: list[AuditResult]) -> None:
    ...
    write_csv(article_root / "outputs" / "tables" / "strategic_foresight_canvas_audit.csv", result_dicts)
    ...
    write_json(article_root / "outputs" / "json" / "strategic_foresight_canvas_export.json", full_export)
