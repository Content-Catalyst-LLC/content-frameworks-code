"""Export utilities for technology and scientific communication Catalyst Canvas integration."""
...
def build_canvas_cards(results: list[AuditResult]) -> list[dict[str, object]]:
    ...
    for result in results:
        cards.append({
            "card_id": f"technology-science-item-{slugify(result.item)}",
            "type": "technology_science_item_card",
            "article_slug": ARTICLE_SLUG,
            "title": result.item,
            "subtitle": result.communication_type,
            "description": result.description,
            "badges": [
                f"priority: {result.review_priority}",
                f"quality: {result.quality_score}",
                f"hype risk: {result.hype_risk}",
                result.status,
            ],
            "metrics": {
                ...
                "method_transparency": result.method_transparency,
                "uncertainty_disclosure": result.uncertainty_disclosure,
                "risk_visibility": result.risk_visibility
            },
            "communication": {
                ...
                "claim_breadth": result.claim_breadth
            },
            "governance": {
                "owner": result.owner,
...
def build_catalog(results: list[AuditResult]) -> dict[str, object]:
    communication_type_counts: dict[str, int] = {}

    for result in results:
        communication_type_counts[result.communication_type] = communication_type_counts.get(result.communication_type, 0) + 1
    ...
    return {
        ...
        "outputs": {
            "audit_table": "outputs/tables/technology_science_canvas_audit.csv",
            "canvas_cards": "canvas/canvas_cards.json",
            "governance_queue": "canvas/governance_queue.json",
            "full_export": "outputs/json/technology_science_canvas_export.json"
        }
    }
...
def write_markdown_queue(path: Path, queue: list[dict[str, object]]) -> None:
    ...
    lines = [
        "# Technology and Scientific Communication Catalyst Canvas Governance Queue",
        "",
        "Generated review queue for claim quality, evidence gaps, method transparency, uncertainty disclosure, hype risk, and governance status.",
        "",
    ]
...
def export_all(article_root: Path, results: list[AuditResult]) -> None:
    ...
    write_csv(article_root / "outputs" / "tables" / "technology_science_canvas_audit.csv", result_dicts)
    ...
    write_json(article_root / "outputs" / "json" / "technology_science_canvas_export.json", full_export)
