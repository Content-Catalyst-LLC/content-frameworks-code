"""Export utilities for Framework Limits Catalyst Canvas integration."""
...
def build_canvas_cards(results: list[AuditResult]) -> list[dict[str, object]]:
    ...
    for result in results:
        cards.append({
            "card_id": f"framework-limits-item-{slugify(result.item)}",
            "type": "framework_limits_item_card",
            ...
            "badges": [
                ...
                f"distortion risk: {result.distortion_risk}",
                result.status,
            ],
            "metrics": {
            ...
            "limits": {
                ...
                "oversimplification_risk": result.oversimplification_risk,
                "false_precision_risk": result.false_precision_risk,
                "context_loss": result.context_loss,
                "audience_burden": result.audience_burden,
                "value_opacity": result.value_opacity
            },
            "governance": {
                "owner": result.owner,
...
def build_catalog(results: list[AuditResult]) -> dict[str, object]:
    ...
    return {
        ...
        "outputs": {
            "audit_table": "outputs/tables/framework_limits_canvas_audit.csv",
            "canvas_cards": "canvas/canvas_cards.json",
            "governance_queue": "canvas/governance_queue.json",
            "full_export": "outputs/json/framework_limits_canvas_export.json"
        }
    }
...
def write_markdown_queue(path: Path, queue: list[dict[str, object]]) -> None:
    ...
    lines = [
        "# The Limits of Framework Thinking Catalyst Canvas Governance Queue",
        "",
        "Generated review queue for clarity, fit, evidence alignment, assumption transparency, governance readiness, oversimplification risk, false precision risk, context loss, audience burden, value opacity, and distortion risk.",
        "",
    ]
    ...
    if not queue:
        ...
        for item in queue:
            lines.extend([
                ...
                f"- Usefulness score: {item['usefulness_score']}",
                f"- Distortion risk: {item['distortion_risk']}",
...
def export_all(article_root: Path, results: list[AuditResult]) -> None:
    ...
    write_csv(article_root / "outputs" / "tables" / "framework_limits_canvas_audit.csv", result_dicts)
    ...
    write_json(article_root / "outputs" / "json" / "framework_limits_canvas_export.json", full_export)
