...
Companion code for the Content Frameworks article **“Persona Frameworks and Their Limits.”**
...
```bash
PYTHONPATH=python python3 -m persona_canvas.cli --article-root .
```

Or:

```bash
python3 python/run_persona_canvas_audit.py
```
...
- `outputs/tables/persona_canvas_audit.csv`
- `outputs/json/catalog.json`
- `outputs/json/canvas_cards.json`
- `outputs/json/governance_queue.json`
- `outputs/json/persona_canvas_export.json`
- `outputs/markdown/governance_queue.md`

## Purpose

The module treats persona frameworks as auditable content-framework assets. It evaluates evidence strength, specificity, content fit, segment alignment, governance readiness, stereotype risk, exclusion risk, weak evidence risk, overgeneralization risk, and review priority.
