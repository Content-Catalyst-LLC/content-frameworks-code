"""Validation helpers for Persona Frameworks Catalyst Canvas inputs."""
...
def validate_rows(rows: list[dict[str, str]]) -> list[str]:
    ...
    if not rows:
        return ["Input contains no persona records."]
    ...
    for index, row in enumerate(rows, start=1):
        ...
        if persona:
            normalized = persona.casefold()
            if normalized in seen_personas:
                errors.append(f"{row_label}: duplicate persona '{persona}'.")
            seen_personas.add(normalized)
