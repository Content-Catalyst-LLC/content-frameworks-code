...
from . import ARTICLE_SLUG, ARTICLE_STAGE, ARTICLE_TITLE, MODULE_NAME, __version__
...
def build_catalog(results: list[AuditResult]) -> dict[str, object]:
    ...
    return {
        ...
        "metrics": [
            "content_score", "audience_score", "evidence_score",
            "repository_score", "governance_pressure",
            "ethical_risk", "readiness_score",
        ],
        "outputs": {
    ...
    if not queue:
        ...
        for item in queue:
            lines.extend([
                ...
                f"- Article stage: {item['article_stage']}",
