"""Export utilities for policy explanation and governance communication Catalyst Canvas integration."""
...
def build_canvas_cards(results: list[AuditResult]) -> list[dict[str, object]]:
    ...
    for result in results:
        cards.append({
            "card_id": f"policy-governance-item-{slugify(result.item)}",
            "type": "policy_governance_item_card",
            "article_slug": ARTICLE_SLUG,
            "title": result.item,
            "subtitle": result.policy_area,
            "description": result.description,
            "badges": [
                f"priority: {result.review_priority}",
                f"completeness: {result.completeness_score}",
                f"risk: {result.governance_risk}",
                result.status,
            ],
            "metrics": {
                ...
                "authority_clarity": result.authority_clarity,
            ...
            "policy_governance": {
                ...
                "participation_clarity": result.participation_clarity,
...
def build_catalog(results: list[AuditResult]) -> dict[str, object]:
    ...
    for result in results:
        policy_area_counts[result.policy_area] = policy_area_counts.get(result.policy_area, 0) + 1
    ...
    return {
        ...
        "outputs": {
            "audit_table": "outputs/tables/policy_governance_canvas_audit.csv",
            "canvas_cards": "canvas/canvas_cards.json",
            "governance_queue": "canvas/governance_queue.json",
            "full_export": "outputs/json/policy_governance_canvas_export.json"
        }
    }
...
def write_markdown_queue(path: Path, queue: list[dict[str, object]]) -> None:
    ...
    lines = [
        "# Policy Explanation and Governance Communication Catalyst Canvas Governance Queue",
        "",
        "Generated review queue for policy explanation completeness, evidence gaps, governance risk, stakeholder visibility, accountability coverage, and communication status.",
        "",
    ]
    ...
    if not queue:
        ...
        for item in queue:
            lines.extend([
                ...
                f"- Policy area: {item['policy_area']}",
                f"- Owner: {item['owner']}",
                f"- Status: {item['status']}",
                f"- Review date: {item['review_date']}",
                f"- Completeness score: {item['completeness_score']}",
                f"- Evidence gap: {item['evidence_gap']}",
                f"- Governance risk: {item['governance_risk']}",
...
def export_all(article_root: Path, results: list[AuditResult]) -> None:
    ...
    write_csv(article_root / "outputs" / "tables" / "policy_governance_canvas_audit.csv", result_dicts)
    ...
    write_json(article_root / "outputs" / "json" / "policy_governance_canvas_export.json", full_export)
