"""Export utilities for OKRs KPIs Measurement Frameworks Catalyst Canvas integration."""
...
def build_canvas_cards(results: list[AuditResult]) -> list[dict[str, object]]:
    ...
    for result in results:
        cards.append({
            "card_id": f"measurement-item-{slugify(result.item)}",
            "type": "measurement_item_card",
            "article_slug": ARTICLE_SLUG,
            "title": result.item,
            "subtitle": result.measurement_type,
            "description": result.description,
            "badges": [
                f"priority: {result.review_priority}",
                f"quality: {result.quality_score}",
                f"risk: {result.measurement_risk}",
                result.status,
            ],
            "metrics": {
            ...
            "measurement": {
                ...
                "gaming_risk": result.gaming_risk,
                "reporting_burden": result.reporting_burden,
...
def build_catalog(results: list[AuditResult]) -> dict[str, object]:
    ...
    for result in results:
        measurement_type_counts[result.measurement_type] = measurement_type_counts.get(result.measurement_type, 0) + 1
    ...
    return {
        ...
        "outputs": {
            "audit_table": "outputs/tables/measurement_canvas_audit.csv",
            "canvas_cards": "canvas/canvas_cards.json",
            "governance_queue": "canvas/governance_queue.json",
            "full_export": "outputs/json/measurement_canvas_export.json"
        }
    }
...
def write_markdown_queue(path: Path, queue: list[dict[str, object]]) -> None:
    ...
    lines = [
        "# OKRs KPIs Measurement Frameworks Catalyst Canvas Governance Queue",
        "",
        "Generated review queue for metric quality evidence gaps measurement risk and governance status.",
        "",
    ]
    ...
    if not queue:
        ...
        for item in queue:
            lines.extend([
                ...
                f"- Measurement type: {item['measurement_type']}",
                f"- Owner: {item['owner']}",
                f"- Status: {item['status']}",
                f"- Review date: {item['review_date']}",
                f"- Quality score: {item['quality_score']}",
                f"- Measurement risk: {item['measurement_risk']}",
...
def export_all(article_root: Path, results: list[AuditResult]) -> None:
    ...
    write_csv(article_root / "outputs" / "tables" / "measurement_canvas_audit.csv", result_dicts)
    ...
    write_json(article_root / "outputs" / "json" / "measurement_canvas_export.json", full_export)
