"""Validation helpers for OKRs KPIs Measurement Frameworks Catalyst Canvas inputs."""
...
def validate_rows(rows: list[dict[str, str]]) -> list[str]:
    ...
    if not rows:
        return ["Input contains no measurement items."]
    ...
    for index, row in enumerate(rows, start=1):
        ...
        measurement_type = str(row.get("measurement_type", "")).strip()
        if measurement_type and measurement_type not in VALID_MEASUREMENT_TYPES:
            errors.append(f"{row_label}: measurement_type '{measurement_type}' must be one of {sorted(VALID_MEASUREMENT_TYPES)}.")
