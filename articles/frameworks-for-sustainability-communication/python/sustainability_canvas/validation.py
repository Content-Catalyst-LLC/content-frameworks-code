"""Validation helpers for Sustainability Communication Catalyst Canvas inputs."""
...
def validate_rows(rows: list[dict[str, str]]) -> list[str]:
    ...
    if not rows:
        return ["Input contains no sustainability claims."]
    ...
    for index, row in enumerate(rows, start=1):
        ...
        if claim:
            ...
            if normalized in seen_claims:
                errors.append(f"{row_label}: duplicate claim '{claim}'.")
