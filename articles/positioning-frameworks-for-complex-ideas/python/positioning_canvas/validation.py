"""Validation helpers for Positioning Frameworks Catalyst Canvas inputs."""
...
def validate_rows(rows: list[dict[str, str]]) -> list[str]:
    ...
    if not rows:
        return ["Input contains no positioning records."]
    ...
    for index, row in enumerate(rows, start=1):
        ...
        if idea:
            ...
            if normalized in seen_ideas:
                errors.append(f"{row_label}: duplicate idea '{idea}'.")
