"""Export utilities for Positioning Frameworks Catalyst Canvas integration."""
...
def build_canvas_cards(results: list[AuditResult]) -> list[dict[str, object]]:
    ...
    for result in results:
        cards.append({
            "card_id": f"positioning-record-{slugify(result.idea)}",
            "type": "positioning_framework_card",
            "article_slug": ARTICLE_SLUG,
            "title": result.idea,
            "subtitle": result.category_frame,
            ...
            "positioning": {
                ...
                "category_clarity": result.category_clarity,
...
def build_catalog(results: list[AuditResult]) -> dict[str, object]:
    return {
        ...
        "outputs": {
            "audit_table": "outputs/tables/positioning_canvas_audit.csv",
            "canvas_cards": "canvas/canvas_cards.json",
            "governance_queue": "canvas/governance_queue.json",
            "full_export": "outputs/json/positioning_canvas_export.json"
        }
    }
...
def write_markdown_queue(path: Path, queue: list[dict[str, object]]) -> None:
    ...
    lines = [
        "# Positioning Frameworks Catalyst Canvas Governance Queue",
        "",
        "Generated review queue for category clarity evidence gaps boundary language drift risk ethical review and governance readiness.",
        "",
    ]
    ...
    if not queue:
        ...
        for item in queue:
            lines.extend([
                ...
                f"- Category frame: {item['category_frame']}",
                f"- Audience pathway: {item['audience_pathway']}",
                f"- Owner: {item['owner']}",
...
def export_all(article_root: Path, results: list[AuditResult]) -> None:
    ...
    write_csv(article_root / "outputs" / "tables" / "positioning_canvas_audit.csv", result_dicts)
    ...
    write_json(article_root / "outputs" / "json" / "positioning_canvas_export.json", full_export)
