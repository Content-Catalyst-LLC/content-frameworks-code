"""Export utilities for Framework Governance Catalyst Canvas integration."""
...
def build_canvas_cards(results: list[AuditResult]) -> list[dict[str, object]]:
    ...
    for result in results:
        cards.append({
            "card_id": f"framework-governance-item-{slugify(result.item)}",
            "type": "framework_governance_item_card",
            ...
            "badges": [
                ...
                f"maturity: {result.governance_maturity}",
            ...
            "metrics": {
                ...
                "ownership_clarity": result.ownership_clarity,
                "review_cycle_strength": result.review_cycle_strength,
                "metadata_completeness": result.metadata_completeness,
                "evidence_status": result.evidence_status,
                "link_health": result.link_health,
                "taxonomy_alignment": result.taxonomy_alignment,
...
def build_catalog(results: list[AuditResult]) -> dict[str, object]:
    ...
    return {
        ...
        "outputs": {
            "audit_table": "outputs/tables/framework_governance_canvas_audit.csv",
            "canvas_cards": "canvas/canvas_cards.json",
            "governance_queue": "canvas/governance_queue.json",
            "full_export": "outputs/json/framework_governance_canvas_export.json"
        }
    }
...
def write_markdown_queue(path: Path, queue: list[dict[str, object]]) -> None:
    ...
    lines = [
        "# Framework Governance and Editorial Maintenance Catalyst Canvas Governance Queue",
        "",
        "Generated review queue for ownership clarity, review-cycle strength, metadata completeness, evidence status, link health, taxonomy alignment, platform readiness, stale evidence risk, dependency complexity, audience impact, and maintenance risk.",
        "",
    ]
    ...
    if not queue:
        ...
        for item in queue:
            lines.extend([
                ...
                f"- Governance maturity: {item['governance_maturity']}",
...
def export_all(article_root: Path, results: list[AuditResult]) -> None:
    ...
    write_csv(article_root / "outputs" / "tables" / "framework_governance_canvas_audit.csv", result_dicts)
    ...
    write_json(article_root / "outputs" / "json" / "framework_governance_canvas_export.json", full_export)
