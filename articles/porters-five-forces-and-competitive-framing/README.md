...
Companion code for the Content Frameworks article **“Porter’s Five Forces and Competitive Framing.”**
...
```bash
PYTHONPATH=python python3 -m five_forces_canvas.cli --article-root .
```

Or:

```bash
python3 python/run_five_forces_canvas_audit.py
```
...
- `outputs/tables/five_forces_canvas_audit.csv`
- `outputs/json/catalog.json`
- `outputs/json/canvas_cards.json`
- `outputs/json/governance_queue.json`
- `outputs/json/five_forces_canvas_export.json`
- `outputs/markdown/governance_queue.md`

## Purpose

The module treats Porter’s Five Forces as an auditable competitive-framing asset. It evaluates force type, market boundary, intensity, evidence strength, uncertainty, strategic relevance, actionability, claim strength, evidence gaps, governance priority, and review status.
